...
- MoveGenerator:
  ...
  - The same idea can be applied to the quality of moves generated. [Pseudo legal moves]() are enough, because verifying that a move is legal can be redundant with playing the move. For instance, in Chess, a classical way to check if a move leaves the king safe is to play the move, then verify that king is safe, then roll back the move. It is more efficient to generate a move that just "seems" legal. If the ai needs to play that move, then it calls the move generator to play the move. The move generator then verifies the king is safe. If not, the move generator method can roll back the move and return a boolean to inform the caller the move is not legal.
